...
## Learning goals
1.	Understand the importance identifying data patterns
...
####	Distributions 
Before looking at the relationships and patterns of our data and variables, it may be useful to get a “feeling” for the key variables in the dataset and visualize some characteristics of these variables. For these visual insights, we typically rely on histograms and box plots to understand the distribution of each variable.

A **histogram** is a type of graph that displays the distribution of a continuous variable. The variable is divided into a set of intervals, or bins, and the frequency or count of the variable falling within each bin is displayed as a bar. Histograms are commonly used to identify the shape and spread of a dataset.
...
Using this histogram, you can first of all see whether there is any interesting variation in the data useful for further statistical analysis. E.g., if all age bins would contain the same number of customers, there is no age variation that can be used to explain, e.g., customer behavior. However, we do not need to worry in this example as we can identify variation from the age bins.

Furthermore, from a business decision perspective, you can identify any patterns that may exist in the age distribution of your customer base. In this example, you see that most customers are between 42 and 56 years old. This information can help guide marketing efforts towards the specific age group that is most likely to purchase your products or services. At the same time, if you believe that your product or service should theoretically be very interesting for people of the age around 35, you could further investigate reasons why people in that age category do not buy the product as much.
...
![Box plot](pics/C2/boxplot.png){height="300px"}
...
![Scatter plot](pics/C2/scattercluster.png){height="300px"}
...
![Scatter plot](pics/C2/corrscp2.png){height="300px"}


![Scatter plot](pics/C2/corrscp1.png){height="300px"}
...
#####	Outliers
As outliers can have a significant impact on the results of statistical tests and models, we would also like to briefly mention statistical techniques to identify such outliers.
...
![Predictive validity framework](pics/C2/pvf.png){height="180px"}
...
Updating your data sources regularly can help mitigate the issue of outdated information. By incorporating the most recent data available, you can ensure that your analysis remains relevant and accurate. This may involve subscribing to data feeds, setting up automated data updates, or maintaining a schedule for manual data updates.
...
###	Noisy data
Noisy data can hinder pattern identification because it obscures the underlying patterns, making it difficult to identify trends or relationships. Dealing with noisy data often requires additional preprocessing steps.
...
Normalization (standardization) and scaling can pose challenges in pattern identification because features with different scales or units may need to be adjusted to enable comparison or to prevent certain features from dominating the analysis or modeling process. Common methods to address these issues are Min-Max Scaling, Z-score Standardization, or Log Transformations.
Min-Max scaling is a technique that transforms the features by scaling them to a specific range, usually [0, 1]. This method is particularly useful when you want to preserve the original distribution of the data while ensuring that all features have the same scale. The formula for Min-Max scaling is: scaled_value = (original_value - min_value) / (max_value - min_value)

Z-score standardization, also known as standard scaling or zero-mean unit-variance scaling, transforms the features by centering them around the mean with a standard deviation of 1. This method is useful when the data follows a normal or Gaussian distribution and you want to compare features that have different units or scales. The formula for Z-score standardization is: standardized_value = (original_value - mean) / standard_deviation

Log transformation is a technique that applies a logarithmic function to the data, effectively reducing the impact of outliers and compressing the range of the data. This method is particularly useful when dealing with data that follows a power-law distribution or when the data has a skewed distribution. The transformation can be applied using any logarithmic base, but the natural logarithm (base e) and base 10 are commonly used:
Choosing the appropriate normalization or scaling method can be difficult, and an improper choice can lead to misleading results or model performance issues.
